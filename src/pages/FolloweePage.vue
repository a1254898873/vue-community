    ...
    <div class="container">
      ...
      <ul class="list-unstyled">
        <li class="media pb-3 pt-3 mb-3 border-bottom position-relative" :key="index"
            v-for="(element,index) in followeeList">
...
export default {
  setup() {
    ...
    const getFolloweeList = () => {
      axios.get('/user/followees/' + state.userId + '/?current=' + state.page.current + '&limit=' + state.page.limit).then((res) => {
        state.followeeList = res.users
        state.user = res.user
        console.log(state.followeeList)
      })
    }
